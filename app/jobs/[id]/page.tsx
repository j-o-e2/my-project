  ...
  if (error) throw error
      ...
      if (data) {
        ...
        try {
          ...
          data.profiles = profileData || { full_name: "Anonymous", avatar_url: null }
        } catch (e) {
          console.error("Error fetching poster profile:", e)
          data.profiles = { full_name: "Anonymous", avatar_url: null }
        }
      }
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <Card className="p-6">
              ...
              {/* Only show poster contact details to the job owner or to the worker whose application was accepted */}
              {(() => {
                ...
                if (isOwner || (isAcceptedWorker && (userApplication?.client_contact_revealed || false))) {
                  return (
                ...
                // If application accepted but contact not yet revealed, show a button to reveal
                if (isAcceptedWorker && !userApplication?.client_contact_revealed) {
                  return (
                    <div className="p-4 bg-muted/10 rounded">
                      <p className="text-sm text-foreground">Client details are hidden</p>
                      <p className="text-xs text-muted-foreground">Your application was accepted. Click the button below to reveal the poster's contact details.</p>
                      <div className="mt-3">
                        <Button
                          onClick={async () => {
                            try {
                              const res = await fetch(`/api/job-applications/${userApplication.id}/reveal`, { method: 'POST', headers: { 'Content-Type': 'application/json' } })
                              const json = await res.json()
                              if (!res.ok) {
                                alert('Failed to reveal contact: ' + (json?.error || 'Unknown'))
                                return
                              }
                              // Refresh job and application state after reveal so UI
                              // reflects any DB changes (helps catch missing skills)
                              try {
                                await fetchJobDetails()
                              } catch (e) {
                                console.warn('Failed to refresh job details after reveal:', e)
                              }
                              await checkUserApplication()
                              alert('Poster contact revealed')
                            } catch (e) {
                              console.error('Reveal failed', e)
                              alert('Failed to reveal poster contact')
                            }
                          }}
